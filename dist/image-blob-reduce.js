...
(function (global, factory) {
  ...
  utils$3.assign = function assign(to) {
    var from;
  ...
  utils$3.pick = pick;
  utils$3.pick_pica_resize_options = pick_pica_resize_options;

  /*! pica 9.0.1 https://github.com/nodeca/pica @license MIT */
  /*
  object-assign
  (c) Sindre Sorhus
  @license MIT
  */
  ...
  var base64decode$1 = function base64decode(str) {
  ...
  var base64decode   = base64decode$1;
  var hasWebAssembly = wa_detect;
  ...
  var unsharp_mask = function unsharp(img, width, height, amount, radius, threshold) {
    var v1, v2, vmul;
    var diff, iTimes4;
  ...
  var FILTER_INFO = resize_filter_info;
  ...
  //
  function convolveHor$1(src, dest, srcW, srcH, destW, filters) {
  ...
  //
  function convolveVert$1(src, dest, srcW, srcH, destW, filters) {
  ...
  //
  function convolveHorWithPre$1(src, dest, srcW, srcH, destW, filters) {
  ...
  //
  function convolveVertWithPre$1(src, dest, srcW, srcH, destW, filters) {
  ...
  var convolve = {
    convolveHor: convolveHor$1,
    convolveVert: convolveVert$1,
    convolveHorWithPre: convolveHorWithPre$1,
    convolveVertWithPre: convolveVertWithPre$1
  };

  const createFilters$1        = resize_filter_gen;
  const { convolveHor, convolveVert, convolveHorWithPre, convolveVertWithPre } = convolve;
  ...
  var resize = function resize(options) {
    ...
    const filtersX = createFilters$1(filter, srcW, destW, scaleX, offsetX),
          filtersY = createFilters$1(filter, srcH, destH, scaleY, offsetY);
  ...
  const createFilters = resize_filter_gen;
  ...
  const mm_unsharp_mask = mm_unsharp_mask$1;
  const mm_resize       = mm_resize$1;
  ...
  MathLib$1.prototype = Object.create(Multimath.prototype);
  MathLib$1.prototype.constructor = MathLib$1;
  ...
  var stepper = function createStages(fromWidth, fromHeight, toWidth, toHeight, srcTileSize, destTileBorder) {
  ...
  const createStages  = stepper;
  const createRegions = tiler;
  const filter_info   = resize_filter_info;
  ...
  function Pica(options) {
    ...
    this.__limit = singletones[limiter_key] || utils$2.limiter(this.options.concurrency);
  ...
  Pica.prototype.init = function () {
    ...
    let initMath = this.__mathlib.init().then(mathlib => {
      ...
      assign$2(this.features, mathlib.features);
    });
    ...
    if (!CAN_CREATE_IMAGE_BITMAP) {
      ...
      checkCibResize = utils$2.cib_support(this.options.createCanvas).then(status => {
    ...
    CAN_USE_CANVAS_GET_IMAGE_DATA = utils$2.can_use_canvas(this.options.createCanvas);

    let checkOffscreenCanvas;

    if (CAN_CREATE_IMAGE_BITMAP && CAN_NEW_IMAGE_DATA && features.indexOf('ww') !== -1) {
      checkOffscreenCanvas = utils$2.worker_offscreen_canvas_support();
    } else {
    ...
    // https://bugs.chromium.org/p/chromium/issues/detail?id=1220671
    let checkCibRegion = utils$2.cib_can_use_region().then(
  ...
  Pica.prototype.__extractTileData = function (tile, from, opts, stageEnv, extractTo) {
    if (this.features.ww && CAN_USE_OFFSCREEN_CANVAS &&
        ...
        (utils$2.isCanvas(from) || CAN_USE_CIB_REGION_FOR_IMAGE)) {
  ...
  Pica.prototype.__tileAndResize = function (from, to, opts) {
    ...
    return Promise.resolve().then(() => {
      ...
      if (utils$2.isCanvas(from)) return null;

      if (utils$2.isImageBitmap(from)) {
        stageEnv.srcImageBitmap = from;
        stageEnv.isImageBitmapReused = true;
        return null;
      }

      if (utils$2.isImage(from)) {
  ...
  Pica.prototype.__resizeViaCreateImageBitmap = function (from, to, opts) {
    ...
    return createImageBitmap(from, {
      ...
      resizeQuality: utils$2.cib_quality_name(filter_info.f2q[opts.filter])
    })
    .then(imageBitmap => {
      if (opts.canceled) return opts.cancelToken;
  ...
  Pica.prototype.resize = function (from, to, options) {
    ...
    let opts = assign$2({}, DEFAULT_RESIZE_OPTS);

    if (!isNaN(options)) {
      opts = assign$2(opts, { quality: options });
    } else if (options) {
      opts = assign$2(opts, options);
    }
  ...
  Pica.prototype.resizeBuffer = function (options) {
    const opts = assign$2({}, DEFAULT_RESIZE_OPTS, options);
